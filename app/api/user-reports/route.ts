...
const schema = z.object({
  ...
  reason: z.enum(["harassment", "inappropriate/harmful", "spam/fake", "impersonation", "other"]),
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      await (convex as any).mutation("userReports:create", {
        name: parsed.data.name,
        email: parsed.data.email,
        reason: parsed.data.reason,
        description: parsed.data.description,
        reportedUsername: parsed.data.username,
        ip,
      });
    } catch (err: any) {
