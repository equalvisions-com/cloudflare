...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // UserId is required - this is the profile being viewed
    if (!userId) {
    ...
    // Step 1: Fetch user activity from Convex with pagination
    ...
    if (guids.length > 0) {
      try {
        ...
        console.error("Error searching activity entries:", error);
      }
    }
    ...
    return NextResponse.json({
      ...
      commentReplies: result.commentReplies || {},
      commentLikes: result.commentLikes || {}
    });
  } catch (error) {
    console.error('Error searching user activity:', error);
    return NextResponse.json(
      { error: 'Failed to search user activity' },
      { status: 500 }
    );
  }
}