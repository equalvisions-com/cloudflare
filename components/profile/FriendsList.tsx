  ...
  return (
    <Drawer open={open} onOpenChange={setOpen}>
      <DrawerTrigger asChild>
        <Button variant="link" className="p-0 h-auto text-sm flex items-center gap-1 focus-visible:ring-0 focus:outline-none hover:no-underline text-muted-foreground font-medium">
          <span className="leading-none">{count}</span><span className="leading-none"> {count === 1 ? "Friend" : "Friends"}</span>
        </Button>
      </DrawerTrigger>
                  ...
                  <div className="flex flex-col flex-1">
                    <Link href={`/@${friend.profile.username}`} onClick={() => setOpen(false)}>
                     ...
                     <Link href={`/@${friend.profile.username}`} onClick={() => setOpen(false)} className="mt-[-4px]">
                      <span className="text-xs text-muted-foreground overflow-anywhere">@{friend.profile.username}</span>
                     </Link>
                  </div>
                  <SimpleFriendButton
                    username={friend.profile.username}
